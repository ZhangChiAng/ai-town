"""Captured-wire tests for the MiniMax Responses Pydantic AI adapter."""
...
from app.model_backends import minimax_responses as adapter_module
...
API_KEY = "minimax-secret-never-preview"
...
async def _make_backend(
    ...
    return await create_minimax_responses_backend(
        FakeSettings(),
        transport=transport,
    )
...
def _contract_case() -> BackendContractCase:
    """Build a fresh real-SDK MiniMax contract fixture."""
...
class TestMiniMaxResponsesBackendContract(BackendContractTests):
    """Apply the neutral async backend contract to MiniMax Responses."""
...
def test_wire_preserves_full_history_and_stateless_boundaries() -> None:
    """The actual body is complete, stateless, and vendor-minimal."""
    transport = RecordingTransport(_successful_response())
...
def test_factory_error_before_client_is_sanitized(
    ...
    """A capture-client constructor error has no sensitive context."""
    ...
    with pytest.raises(
        RuntimeError,
        match="^MiniMax Responses client creation failed$",
    ) as exc_info:
        asyncio.run(create_minimax_responses_backend(FakeSettings()))
...
def test_factory_failure_closes_captured_client_once(
    ...
    """Every factory failure after HTTP client creation closes it first."""
