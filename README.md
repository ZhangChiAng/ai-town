...
工具选择，模型直接生成一段非空普通文本作为消息正文。接收者不由模型决定：
每次生成或重新生成后均保持未选择，必须由用户在确认发送前手动选择。

发给模型的上下文只有：

- `system`：当前 Agent 已保存的 `system_prompt`，逐字作为唯一角色提示；
- 当前 Agent 自己的已确认个人时间线，逐条映射为原生 Messages API 消息：
  收到的记录使用 `user` 角色和 `{counterpart_id}：{content}`，发出的记录
  使用 `assistant` 角色并只保留正文；内心声音使用
  `user` 角色和固定文本 `内心的声音：{content}`。

每条时间线记录保持独立的文本块和原有顺序；连续同角色记录会放入同一个 API
turn，以兼容要求 `user`/`assistant` 严格交替的 Anthropic-compatible 网关。
如果时间线为空，或最后一个 API turn 是已发送消息形成的 `assistant`，请求会
在末尾追加运行时 `user` 消息 `现在轮到你说话。`，避免空 `messages` 和
assistant prefill；收到消息或内心声音形成的 `user` 已在末尾时不追加。该触发
语不写入时间线，也不作为缓存断点。四个拼接槽位不会在 user context 中重复
发送。身份与候选关系应由用户编辑的 system prompt 提供。场景名、其他 Agent
的私有设定和时间线、观察者信息、未确认草稿也不会进入请求。生成只覆盖浏览器
内当前 Agent 的临时草稿，不修改场景 JSON。用户可以编辑正文或完全手写；选择
接收者后才能点击“确认发送”，把匹配记录原子写入发送者和接收者的时间线。
...
每个 Agent 的“JSON 请求预览”从已保存场景构造，不调用模型、不修改 JSON；
场景有未保存修改时界面明确标记该预览为旧版本。预览以格式化、自动换行的
JSON 展示 Anthropic Messages API 模型载荷，包括模型名、token 上限、system、
按请求顺序排列的实际无工具消息，但不包括 API Key、Base URL 或第三方响应。
提示词和请求不会写入日志。
