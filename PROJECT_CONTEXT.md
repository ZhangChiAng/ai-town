...
- `prompt_profile`：代词、不可说出口的信念、内层记忆与外层记忆四个可编辑
  变量；
- `interactions`：该 Agent 面向其他 Agent 的“称呼 → 使用场合”有序字典；
...
键只能是另外两个现有 Agent ID。手工事件完全不解析、不改写，因此用户可以
直接输入 `儿子对你说：正文` 来模拟回复。
...
当前 scene schema 为 v9。schema v1–v8 一律拒绝，不迁移、不兼容，也不改写
原文件；用户必须创建新 v9 场景并重新录入变量与互动关系。
...
失效。它不哈希 adapter payload 或 reasoning；请求字段、缓存传输元数据或
reasoning 变化不会单独使草稿失效。
...
时，确认返回 `409`。接收者显示名不进入语义历史，单独改名不会改变发送方的
上一轮输入。无效的用户编辑外层输出返回 `422`。
...
`outer_memories`，后端在每次预览、生成、确认和状态令牌重建时使用以下唯一
固定模板。前端不复制模板。
...
场景允许保存四个空变量和空互动字典，B/C 可以完全不配置。某 Agent 只有在
四个变量均非空且至少配置一个互动称呼时，才能预览、生成或确认任一层；否则
返回 `409` 且不调用模型。
...
因此进入浏览器响应、reasoning 投影、确认数据、scene v9 或后续模型上下文。
...
`{model}`。scene schema 当前为 v9，不加载或迁移 v1–v8，并继续只用
`scene.model` 保存绑定。
...
`data/scenes/`，只在其中保存 schema v9 场景 JSON。
